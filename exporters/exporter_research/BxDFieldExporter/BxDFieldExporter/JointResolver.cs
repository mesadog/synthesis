        ...
        private byte[] ProcessRotationalJoint(RigidBodyJoint rotationalJoint)
        {
            try
            {
                ...
                foreach (AssemblyJoint joint in rotationalJoint.Joints)
                {
                    ...
                    BitConverter.GetBytes((float)vectorCircle.Normal.X).CopyTo(vectorJointData, 0);
                    BitConverter.GetBytes((float)vectorCircle.Normal.Y).CopyTo(vectorJointData, 4);
                    BitConverter.GetBytes((float)vectorCircle.Normal.Z).CopyTo(vectorJointData, 8);
